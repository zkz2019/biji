        ...
        <template v-if="type != 1">
          <div slot="left" class="left-tree">
            <fel-tree1
              ...
              :idArr="[0]"
              :refresh="refreshTree"
              interface="/system/device/deviceyxlock/1/getbuildtree"
      ...
      <el-container>
          ...
          <retrieval class="query_head">
            ...
            <inpbox :inpb="true">
              <el-input
                ...
                placeholder="输入位置/房间名称/唯一ID查询"
              ></el-input>
            </inpbox>
            <inpbox>
              <fel-button class="qh_btn" type="primary" @click="onRefreshSearch">查询</fel-button>
        ...
        <el-main class="query_main">
          <paging-table
            ...
            interface="/system/device/deviceyxlock/2/getdevicelock"
            :list="list"
        ...
        ></replaceWired>
        <infoWired :param="infoParam" @beforeClose="dialogInfo = false" :dialogVisible="dialogInfo"></infoWired>
        <guideFile
          title="有线联网锁"
          :dialogVisible="dialogGuideFile"
          historyUrl="/system/device/deviceyxlock/upload/8/downhistory"
...
import queryWired from "./queryWired";
import historyWired from "./historyWired";
import modifyWired from "./modifyWired";
import replaceWired from "./replaceWired";
import infoWired from "./infoWired";
    ...
    return {
      ...
      list: [
        ...
        },
        {
          ...
          prop: "roomdate",
          formatter(row) {
            return row.roomstate == "未安装" || row.roomstate == "未初始化"
              ? ""
              : row.roomdate;
          },
        },
  ...
  methods: {
    routeSearch() {
      if (
        ...
        this.toRoute.params.to == "sy"
      ) {
        this.codeNext = true;
        this.gatewaycodeArr = this.toRoute.params.code;
        this.paramObj.searchList = this.gatewaycodeArr;
      } else {
        this.gatewaycodeArr = [];
        this.paramObj.searchList = [];
      }
      this.onRefresh();
    ...
    inGetlockstate() {
      this.$ajax("/system/device/deviceyxlock/3/getlockstate", {}, "1")
        .then((res) => {
    ...
    async inSavegatewayorder(id, obj) {
      let next = true;
      if (id == "484") {
        await this.$confirm(
          "该指令会导致正常工作的门锁设备立即脱网，使用该指令时需确认门锁安装点有经过培训的系统维护员存在，否则设备将一直处于异常状态，确定下发该指令吗？",
          "提示",
          {
      ...
      if (id == "565") {
        ...
        this.$ajax(
          "/system/device/deviceyxlock/order/1/savelockorder",
          obj,
          "1",
                ...
                arr.forEach((obj) => {
                  ...
                  if (vid == "488") {
                    this.importButs.push({
                      ...
                      tempUrl: "",
                      url:
                        "/system/device/deviceyxlock/upload/2/uploaddevicelock",
                      errUrl:
                        "/system/device/deviceyxlock/upload/6/downfaildevicelock",
                    });
                  } else if (vid == "489") {
                    this.exportButs.push({
                      ...
                      name: valias,
                      url:
                        "/system/device/deviceyxlock/upload/7/downdevicelocks",
                      data: this.param,
                    });
                  } else if (vid == "482") {
    ...
    delete(arr) {
      this.$confirm("此操作将删除当前门锁, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$ajax(
            "/system/device/deviceyxlock/delete/1/deletedevicelock",
            {},
            "1",
            arr,
            true
    ...
    modify(data, bool = false) {
      ...
      this.modifyParam.type = this.type == 0 ? "1" : "2";
    ...
    onEjectChange() {
      this.$common.onEjectChange(this.list, "zzyxws476");
    },
    getEject() {
      this.$common.getEject(this, "list", "zzyxws476");
    },
  },
};
</script>
