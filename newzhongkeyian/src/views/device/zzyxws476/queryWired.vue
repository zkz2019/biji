<template>
  ...
  >
    <el-container class="dialog-table6 query_main">
      <paging-table
        interface="/system/device/deviceyxlock/order/2/getlockorder"
        :list="list"
        :param="paramObj"
        :refresh="refresh"
      />
    ...
    >
      <el-container class="dialog-table6 query_main">
        <paging-table
          interface="/system/device/deviceyxlock/order/3/getlockorderinfo"
          :list="detailedList"
          :param="detailedParam"
    ...
    >
      <el-container class="dialog-table query_main">
        <paging-table
          style="height:100%"
          interface="/system/device/deviceyxlock/order/4/getlockorderchangeinfo"
          :list="changeList"
          :param="detailedParam"
      ...
      changeList: [
        ...
        },
        {
          ...
          template: {
            ...
            methods: {
              getClass() {
                let value = this.scope.row.lostatus;
                if (value == "处理成功") {
                  return "puc-pg";
                } else if (value == "处理失败") {
                  return "puc-px";
            ...
            template: `<span :class='getClass()'>{{scope.row.lostatus}}</span>`
          }
        },
        {
          name: "门锁位置",
  ...
  methods: {
    ...
    inReloadgatewayorder() {
      if (this.detailedParam.goid) {
        this.$ajax(
          "/system/device/devicelock/order/5/reloadlockorder",
