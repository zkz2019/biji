  ...
  >
    <div>
      <div v-if="importButs.length + exportButs.length >= 1">
        <h3 class="htitle">
          选择操作类型
          <el-button class="htitle_history" type="text" @click="onClickHistory()">导出历史记录</el-button>
        </h3>
        <div class="content">
          ...
          <div class="content import">
            ...
            >
              <div class="text">
                ...
                <el-button v-if="importShow" class="com-but-small" type="primary">导入文件</el-button>或，将文件拖到此处
                <div slot="tip" class="el-upload__tip gui_text">只支持后缀名为{{wjType}}的文件，且文件大小不超过100MB</div>
              </div>
            </fel-upload>
            <div class="but">
              <el-button type="primary" class="com-but-small" @click="onUpload">确认导入</el-button>
            </div>
...
import { timerDownload } from "./index.js";
export default {
  ...
  methods: {
    ...
    inExportPackage(url, name, data = {}, obj = {}, down = true) {
      this.$ajax(url, data, "1", obj, true, 60000)
        .then(res => {
          ...
          timerDownload(res.result, this.historyUrl, this, name);
        })
        .catch(err => {
          this.$message({
            showClose: true,
