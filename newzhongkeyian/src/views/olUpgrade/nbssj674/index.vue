...
<template>
  <el-container>
    <fel-left-tree leftTitle="建筑列表" class="nblots361">
      <div slot="left" class="left-tree">
        <fel-tree1
          ...
          :idArr="[0]"
          :refresh="refreshTree"
          interface="/system/onlineup/nbversion/1/getbuild"
      ...
      <el-container>
        ...
        <el-main class="query_main">
          <paging-table
            ...
            interface="/system/onlineup/nbversion/2/getnblock"
            :list="list"
    ...
    <nbVup :dialogVisible="dialogVisible" @beforeClose="dialogVisible = false"></nbVup>
    <nbAutoup
      :stopBtns="stopBtns"
      :dialogVisible="autoupVisible"
      @beforeClose="autoupVisible = false"
    ></nbAutoup>
...
import nbVup from "./nbVup";
import history from "./history";
import nbAutoup from "./nbAutoup";
export default {
  components: { nbVup, history, nbAutoup },
  props: {
    toParam: Object,
    toRoute: Object,
  },
  data() {
    ...
    return {
      ...
      nustates: [
        { id: "", name: "全部状态" },
        // { nustate: "", nuname: "全部" },
        // { nustate: "0", nuname: "正在下发" },
        // { nustate: "1", nuname: "下发成功" },
        // { nustate: "-1", nuname: "下发失败" }
      ],
      // roomtxtypes: [{ roomtxtype: "", typename: "全部运营商门锁" }],
      refreshTree: 0,
      ...
      list: [
        ...
        },
        {
          ...
          template: {
            ...
            template: `<ul @click="onClick(scope.row)" style="cursor:poInter;color:#3a8e"><li>{{scope.row.deviceversion}}</li><li>{{scope.row.nusdate?scope.row.nusdate.substr(0,10):""}}</li></ul>`,
          },
        },
        {
          name: "门锁下载进度",
          ...
          template: {
            ...
            computed: {
              state() {
                let stateList = [];
                if (
                  this.scope.row.nustatename &&
                  this.scope.row.nustatename.length > 3
                ) {
                  stateList.push(this.scope.row.nustatename.substring(0, 3));
                  stateList.push(this.scope.row.nustatename.substring(4));
                } else {
                  stateList.push(this.scope.row.nustatename);
                }
                return stateList;
              },
            },
            methods: {
              getClass() {
                let value = this.scope.row.nustatename;
                if (value == "已同步") {
                  return "puc-pg";
                } else if (value == "下载中") {
                  return "";
              ...
              listBut() {
                ...
                list.forEach((item) => {
                  ...
                  if (
                    ...
                    lists[0].push(item);
                  } else if (item.id == "679" || item.id == "680") {
                    lists[1].push(item);
                  } else if (item.id == "719") {
                    lists[2].push(item);
                  }
  ...
  methods: {
    ...
    getType() {
      this.$ajax("/system/onlineup/nbversion/5/getsearchtype", {}, "1").then(
        (res) => {
    ...
    onStartnblockup(type, row = null) {
      ...
      switch (type) {
        case "1":
          text = "全新下载";
          url = "/system/onlineup/savenbversion/2/startnblockup"; //版本传输
          break;
        case "2":
          text = "续传下载";
          url = "/system/onlineup/savenbversion/4/restartnblockup"; // 版本续传
          break;
        case "3":
          text = "更新升级";
          url = "/system/onlineup/savenbversion/3/nblocktoupversion"; //更新升级
          break;
        case "4":
          text = "强制升级";
          url = "/system/onlineup/savenbversion/5/nblockforceversion"; //强制升级
          break;
        case "5":
          text = "停止下载";
          url = "/system/onlineup/savenbversion/6/deletenblockup"; //强制升级
          break;
        case "6":
          text = "自动升级";
          url = "/system/onlineup/savenbversion/7/saveautonblockup"; //自动升级
          break;
      }
      let data = {
      ...
      this.$ajax("/login/home/2/getsonmenu", { fatherid: id }, "1")
        .then((res) => {
          res.result.forEach((item) => {
            ...
            } else if (entity.id == "677") {
              this.topButs.push(entity);
            } else if (entity.id == "678") {
              this.quantitytypes.push(entity);
              this.listBut.push({
                type: "2",
                id: entity.id,
            ...
            } else if (entity.id == "720") {
              this.topButs.push(entity);
            } else if (entity.id == "928") {
    ...
    getNbversion() {
      this.$ajax("/system/onlineup/savenbversion/1/getnbversion", {}, "1")
        .then((res) => {
