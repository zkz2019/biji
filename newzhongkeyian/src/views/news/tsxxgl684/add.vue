...
export default {
          ...
          buts: [
            {
              ...
              popover: {
                content:
                  "统计推送日当天数据为本周期推送, 统计推送日前一天为上周期推送"
              }
            }
          ]
        },
  ...
  methods: {
    onPushsendtype(arr, data) {
      if (arr[0] == "微信") {
        data.pushRange = "角色范围/人员范围";
      } else if (arr[0] == "邮件") {
        data.pushRange = "角色范围/人员范围/邮件范围";
      } else if (arr[0] == "短信") {
        data.pushRange = "角色范围/人员范围/手机范围";
      }
    },
    pushtypeChange(arr, data, value, is) {
      if (!is) {
        data.pushtype2 = "";
      }
      let id = arr[0];
    ...
    pushtype2Change(arr, data, value, is) {
      if (!is) {
        data.pushparam = "";
      }
      let id = arr[0];
    ...
    onBttype(arr, valeu, data, is) {
      ...
      let type = "number";
      if (arr[0] == "3" || arr[0] == "2") {
        this.formData[8].disabled = false;
        type = "select";
        ...
        this.formData[8].disabled = true;
        valeu.pushcycleparam = "";
      }
      this.formData[8].type = type;
    ...
    inGetrole() {
      ...
      //     this.selects.userrole = result.map(o => o.rolename);
      //   })
      //   .catch(err => {});
    },
    inSelectType() {
    ...
    ingetupdatemsgpush() {
      this.$ajax(
        ...
        .then(res => {
          let obj = res.result || {};
          this.onPushsendtype([obj.pushsendtype], obj);
