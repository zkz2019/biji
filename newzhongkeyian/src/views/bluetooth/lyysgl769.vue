...
export default {
    ...
    return {
      ...
      mjList: [
        ...
        },
        {
          name: "开门时间段",
          formatter(row) {
            return row.auaopenstime + "-" + row.auaopenetime;
          },
        },
        {
          name: "可开门次数",
  ...
  computed: {
    interface() {
      return this.ismj
        ? "access/v2.0/auth/info/c/listAppAccessAuth"
        : "/auth/bluetooehkeycenter/bluetooehkey/deletebluetoothkeys/1/getbluetoothkeyauth";
    },
  },
  methods: {
    inGetsonmenu() {
    ...
    confirmModify(data, obj) {
      ...
      if (this.ismj) {
        let auaids = obj.isAll ? this.roids.map((o) => o.auaid) : [obj.auaid];
        let param = {
          auaids,
          auaempedate: data.auaempedate,
          auaempsdate: data.auaempsdate,
          auaopenetime: data.auaopenetime,
          auaopenstime: data.auaopenstime,
          auausecount: data.auausecount,
          personcode: this.grantParam.personcode,
          type: this.range,
        };
        this.$ajax(
          "/access/v2.0/auth/info/e/updateAppAccessAuth",
          param,
          "9",
          {},
          true
    ...
    onDelete(data, obj, isAll = false) {
      if (this.ismj) {
        ...
        const auaids = data ? [data.auaid] : this.roids.map((o) => o.auaid);
        const param = { auaids, personcode, type };
        this.$ajax(
          "/access/v2.0/auth/info/d/delAppAccessAuth",
          param,
          "9",
          {},
          true
