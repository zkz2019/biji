    ...
    <el-tabs v-model="activeName" @tab-click="handleClick">
      ...
      <el-tab-pane label="功能配置" name="second">
        <fel-form
          ...
          :formData="formData1"
        ></fel-form>
        <div class="config_second_text">同步模式、消防报警将应用到门禁下全部的门，请按需配置</div>
      </el-tab-pane>
      <el-tab-pane label="自动开关门时段" name="third">
        <div v-if="autoData.length === 0" class="config_null">暂无数据</div>
        <div v-else id="autoScroll" class="config_box">
          <template v-for="(obj,key) in autoData">
            <div class="config_box_top" :key="key">
        ...
        <div class="config_bottom">
          <fel-button class="qh_btn com-but-small" @click="onAdd">新增时段</fel-button>
      ...
      <el-tab-pane label="门配置" name="fouth">
        <fel-form
          ref="felForm"
          class="single-row quick-ruleForm"
          :selects="selects"
      ...
      <el-tab-pane label="备注信息" name="fifth">
        ...
        <el-input class="beizhuText" placeholder="输入门的备注信息，如门的别名等" type="textarea" v-model="beizhu"></el-input>
        <div class="config_bottom">
          <fel-button class="qh_btn com-but-small" @click="beforeClose">取消</fel-button>
          <fel-button class="qh_btn com-but-small" type="primary" @click="onSubmit1">确定</fel-button>
        </div>
      </el-tab-pane>
    </el-tabs>
  </fel-dialog>
  ...
  methods: {
    ...
    getAccessMainDeploy() {
      this.$ajax(
        ...
        .then((res) => {
          ...
          console.log(
            "this.defaultDataGNPZ.amsyncauths",
            this.defaultDataGNPZ.amsyncauths
          );
        ...
        this.$ajax(
          ...
          .then((res) => {
            ...
            for (let i = 1; i <= cd; i++) {
              ...
              this.formData.push({
                render(obj, add) {
                  if (obj.heads && obj.heads[i - 1]) {
                    ...
                    if (obj.heads[i - 1].ahstate == "正常") {
                      return add(
                        "div",
                        { class: "puc-pg" },
                        obj.heads[i - 1].ahstate
                      );
                    } else {
                      return add(
                        "div",
              ...
              if (data.heads[i - 1].ahauths.indexOf("指纹") >= 0) {
                is = !is;
                this.formData.push({
                  funct(obj) {
                    if (obj.heads && obj.heads[i - 1]) {
              ...
              if (data.heads[i - 1].ahauths.indexOf("人脸") >= 0) {
                is = !is;
                this.formData.push({
                  funct(obj) {
                    if (obj.heads && obj.heads[i - 1]) {
    ...
    handleClick(data, obj) {
      if (data.name == "first") {
        ...
        this.beizhu = this.param.adremark || "";
      }
    },
    onReturn(str, obj) {
    ...
    onAdd() {
      ...
      let dom = document.getElementById("autoScroll");
      setTimeout(() => {
    ...
    onSubmit1() {
      this.$ajax(
        "/access/v2.0/door/q/updateAccessDoorRemark",
        { adid: this.param.adid, adremark: this.beizhu },
        "9",
        {},
        true
      )
        ...
        .catch((err) => {
          this.$message({
            type: "error",
            message: `[${err.resultCode}] ` + err.resultMsg || "修改备注失败!",
          });
        });
    },
    ...
    submitForm(data) {
      ...
      console.log("amsyncSel", amsyncauths);
      let obj = {
        amid: this.param.amid,
        amsync: data.amsync,
        amwarn: data.amwarn,
