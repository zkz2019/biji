...
export default {
  ...
  methods: {
    ...
    onChange(arr, data) {
      ...
      this.inGetareacode(arr[0], is);
    },
    inSaveroom3(data) {
      if (data.areatype == 4 && data.buildtype != 5) {
        if (!data.houseid) {
          this.$message({
            showClose: true,
            message: "请选择房型",
