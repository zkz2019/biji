  ...
  <el-container>
    <el-header class="elheader query_headbox">
      ...
      <retrieval class="query_head">
        <inpbox :inpb="true">
          <el-select v-model="param.personstate" class="con-select qh_inp">
            <el-option
    ...
    />
    <detailsAdd :dialogVisible="dialogAdd" :param="paramAdd" @beforeClose="dialogAdd = false" />
...
export default {
    ...
    return {
      ...
      mjList: [
        ...
        },
        {
          name: "开门时间段",
          formatter(row) {
            return row.aufopenstime + "-" + row.aufopenetime;
          },
        },
        {
          name: "可开门次数",
      ...
      list: [
        ...
        },
        {
          ...
          template: {
            ...
            template: `<div>
              <span v-if="scope.row.accesscount && scope.row.accesscount != 0"><a class="a-click" @click.stop="onClick">{{scope.row.accesscount}}</a></span>
              <span v-else>{{scope.row.accesscount}}</span>
            </div>`,
          },
        },
        {
          ...
          template: {
            ...
            methods: {
              ...
              getState(str, state) {
                ...
                //   "this.maxfinge",
                //   $this.maxfinge,
                //   this.row.fingercount,
                //   this.row.fingercount < $this.maxfinge
                // );
                if (str == "删除") {
                  return true;
                } else if (
  ...
  computed: {
    interface() {
      return this.ismj
        ? "/access/v2.0/auth/info/6/listFingerAccessAuth"
        : "/auth/fingercenter/finger/deletefingerauths/1/getfingerauths";
    },
  },
  created() {
    ...
    this.maxfinge = Storages.getStorage("maxfingernum");
    this.inGetsonmenu();
  },
  mounted() {
    this.getEject();
  ...
  methods: {
    ...
    onGrantChange(val) {
      if (val == 2) {
        ...
        // this.$refs["grant-paging-table1"].clearSelection();
        // this.$refs["grant-paging-table1"].toggleAllSelection();
        setTimeout(() => {
    ...
    confirmModify(data, obj) {
      if (this.ismj) {
        ...
        let param = {
          ...
          aufempedate: data.aufempedate,
          aufempsdate: data.aufempsdate,
          aufopenetime: data.aufopenetime,
          aufopenstime: data.aufopenstime,
          aufusecount: data.aufusecount,
        };
        this.$ajax(
          "/access/v2.0/auth/info/8/updateFingerAccessAuth",
          param,
          "9",
          {},
          true
    ...
    //选择 修改指纹授权列表
    onCardModify(arr) {
      console.log("arr,this.rcids", arr, this.rcids);
      let data = this.rcids;
      if (arr) {
        data = arr;
      }
    ...
    onGrantDelete(arr) {
      if (this.ismj) {
        ...
        this.$confirm("确定要删除选择的授权吗", "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        }).then(() => {
          this.$ajax(
            "/access/v2.0/auth/info/7/delFingerAccessAuth",
            param,
            "9",
            {},
            true
    ...
    onClick(key, data, obj) {
      if (key == "d1") {
        ...
        this.dialogAdd = true;
        this.paramAdd.personcode = data.personcode;
      } else if (key == 404) {
        this.dialogAdd = true;
        this.paramAdd.personcode = "";
      } else if (key == "g4") {
