  ...
  <div class="con_rowt clearfloat">
    ...
    <div class="rowt_right">
      ...
      <div v-if="states4" class="msgbox">
        ...
        <ul>
          <li>
            <div>{{this.getPersontypes[1]||"学生人数"}}:</div>
            <div>{{this.roominfo.stucount}}</div>
          </li>
          <li>
            <div>{{this.getPersontypes[2]||"教师人数"}}:</div>
            <div>{{this.roominfo.staffcount}}</div>
          </li>
          <li>
            <div>{{this.getPersontypes[3]||"临时人员"}}:</div>
            <div>{{this.roominfo.tempcount}}</div>
          </li>
          <li>
            <div>{{this.getPersontypes[4]||"其他人员"}}:</div>
            <div>{{this.roominfo.jdcount}}</div>
          </li>
        </ul>
      </div>
    </div>
    <syDialog
      ...
      dynamic
    ></syDialog>
    <syDialogMS :dialogVisible="dialogShowMS" @beforeClose="dialogShowMS=false" dynamic></syDialogMS>
  </div>
</template>
...
import syDialogMS from "./syDialogMS";
...
export default {
  ...
  created() {
    ...
    this.getPersontypes = this.getPersontype()();
    console.log("this.getPersontypes", this.getPersontypes);
  },
  watch: {
    states1(arr) {},
    states2(arr) {},
  ...
  destroyed() {},
  methods: {
    ...mapGetters(["getRoomtype2", "getPersontype"]),
    resizeListener(bool) {
      if (bool) {
