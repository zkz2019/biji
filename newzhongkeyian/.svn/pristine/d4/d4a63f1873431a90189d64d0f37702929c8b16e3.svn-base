...
<template>
  <div class="groupingtree">
    <div class="top-buts" v-if="clickData && clickData.data && clickData.data.ammanager == 1">
      ...
      <el-button
        icon="ficon-image804"
        v-if="buts.includes(9) && clickData &&  clickData.data && clickData.data[nodeKey] != '0'"
        type="text"
        size="small"
    ...
    <fel-popup append-to-body form width="50%" title="新增" @close="addClose" :value="add.box">
      <fel-form
        ref="addForm"
    ...
    <fel-popup append-to-body form width="50%" title="重命名" @close="renameClose" :value="rename.box">
      <fel-form
        ref="renameForm"
        dynamic
...
export default {
  ...
  methods: {
    setCurrentKey(key) {
      if (this.$refs["tree1"]) {
        this.$refs["tree1"].setCurrentKey(key);
      }
    },
    renameSubmit(param) {
      this.$ajax("/arearoom/3/updateArea", param, "9", {}, true)
        .then(res => {
          this.$message({
            message: "重命名成功",
            type: "success"
          ...
          node.parent.expand(() => {
            data.areaname = param.areaname;
            node.data.areaname = param.areaname;
