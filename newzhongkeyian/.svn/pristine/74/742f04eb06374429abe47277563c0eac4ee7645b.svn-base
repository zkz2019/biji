...
<template>
  <el-container>
    <fel-left-tree class="building">
      <template slot="left">
        ...
        <grouping
          v-if="type==1"
          :buts="topButGrou"
          :butAdmin="topButAdmin"
          class="tree1"
          @handleNodeClick="groupingClick"
        ></grouping>
      ...
      <classify
        @modify="onModify"
        :topButs="topButsClassify"
        :listButs="listButsClassify"
        v-if="type==1"
        ref="classify"
      ></classify>
      <el-container v-else>
        <el-header class="elheader query_headbox">
          ...
          <retrieval class="query_head" v-if="fatheragid.buildtype==1||fatheragid.buildtype==3">
            ...
            <inpbox :inpb="true">
              <el-select
                class="inputs con-select qh_inp"
        ...
        <el-main class="padt0 query_main">
          ...
          >
            ...
            <template v-if="fatheragid.ismanager == 1 && isBatch">
              <span key="c1494" class="sli but-blue" @click="onClick(494)">
                <i class="ficon-image494"></i>
                {{batchTitle}}
              </span>
            </template>
            <template v-if="fatheragid.ismanager == 1 && isAdd">
              <span key="c28" class="sli but-blue" @click="onClick(28)">
                <i class="ficon-image28"></i>
                {{addTitle}}
              </span>
            </template>
            <template v-if="fatheragid.ismanager == 1 && fatheragid.buildtype!=3 && isCopy">
              <span key="c197" class="sli but-blue" @click="onClick(197)">
                <i class="ficon-image197"></i>
                {{copyTitle}}
              </span>
            </template>
    ...
    <batch-room
      @onRefresh="onRefreshs"
      :dialogVisible="dialogBatchRoom"
      @beforeClose="dialogBatchRoom=false"
      :fatheragid="fatheragid"
    ></batch-room>
  </el-container>
</template>
  ...
  methods: {
    ...
    groupingClick(...arr) {
      if (this.$refs["classify"]) {
        this.$refs["classify"].handleNodeClick(...arr);
      }
    },
    //重置事件
    onReset() {
        ...
        .then((res) => {
          ...
          res.result.forEach((value) => {
            ...
            if (id == "28") {
              ...
              this.listButsClassify.push(value.entity);
              ...
              this.topButGrou.push(1);
            } else if (id == "904") {
              this.topButGrou.push(5);
            } else if (id == "905") {
              this.topButGrou.push(2);
            } else if (id == "906") {
              this.topButGrou.push(9);
              if (value.childs) {
                this.topButAdmin = value.childs.map((o) => o.entity);
              }
            } else if (id == "911") {
              this.topButsClassify.push(value.entity);
            } else if (id == "912") {
              this.topButsClassify.push(value.entity);
            } else if (id == "913") {
              this.listButsClassify.push(value.entity);
    ...
    handleNodeClick(data, nodes) {
      ...
      this.effectTitle = nameObj.layout;
      this.batchTitle = nameObj.batch;
      this.addTitle = nameObj.add;
      this.copyTitle = nameObj.copy;
      this.onRefresh();
    },
    inDelbuilds(data) {
