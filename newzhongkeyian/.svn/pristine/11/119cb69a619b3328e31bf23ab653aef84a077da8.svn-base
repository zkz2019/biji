...
export default {
    ...
    return {
      ...
      formData: [
        ...
        },
        {
          ...
          buts: [
            {
              class: "but-connect ", //这个地方是故意留一个空格
              title: "查询人员信息",
              icon: "el-icon-search",
  ...
  methods: {
    ...
    //获取手指列表
    async getyourfingers(personcode) {
      ...
      if (this.param.selects) {
        this.selects.fgseq = [this.param.selects];
        if (this.$refs.felForm) {
          let fgseq = this.$refs.felForm.ruleForm.fgseq;
          let arr = this.selects.fgseq.map((o) => o.id);
          if (!arr.includes(fgseq)) {
  ...
  watch: {
    dialogVisible(val) {
      if (val) {
        ...
        this.getfingerprint();
        this.getyourfingers(this.param.personcode || "");
        let fpcode = this.isConnect || "";
        if (this.param && this.param.personcode && this.isUpdate) {
          this.disabled = true;
          this.defaultData = {
            fpcode,
            islocal: "1",
          ...
          if (this.param && this.param.personcode) {
            this.defaultData.personcode = this.param.personcode;
          }
        }
        this.formData[1].noShow = false;
