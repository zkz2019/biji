      ...
      <retrieval class="query_head">
        ...
        <inpbox :inpb="true">
          <el-input
            ...
            style="width:300px"
            :placeholder="'输入房间/门禁名称/归属人/'+getNumber()+'查询'"
          ></el-input>
        </inpbox>
        <inpbox>
          <fel-button class="qh_btn" type="primary" @click="search(false)">查询</fel-button>
...
export default {
      ...
      list: [
...

        {
          ...
          template: {
            ...
            computed: {
              name() {
                if (this.scope.row.sendstate == "0") {
                  return "正在下发";
                } else if (this.scope.row.sendstate <= 0) {
                  return "下发失败";
                } else if (this.scope.row.sendstate == "") {
                  return "";
                } else {
                  return "下发成功";
                }
  ...
  activated() {
    ...
    if (obj.devicetype) {
      this.param.devicetype = obj.devicetype;
      this.search(false);
    }
  },
  computed: {
