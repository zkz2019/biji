        ...
        <adjust class="batch-authori">
              ...
              <div class="b-left_box_bottom">
                ...
                <template v-else>
                  <paging-table
                    ...
                    interface="/access/v2.0/auth/d/getFinger"
          ...
          <adjust-div class="b-right" dragLeft>
            ...
            <paging-table
              ...
              interface="/access/v2.0/auth/e/getAccessAuthFinger"
              :list="generateList"
              ajaxType="9"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ryoptions: [],
      listButs: [],
      cardbuild: { areas: [], pgids: [], persons: [], type: "" },
      generateList: [
        {
          name: "序号",
          type: "index",
        ...
        },
        {
          ...
          formatter(row, a, b) {
            return (
              ...
              "-" +
              row.amname +
              (row.adname ? "-" + row.adname : "")
              //  +(row.ahname ? " - " + row.ahname : "")
            );
          },
        },
      ],
    ...
    onReturn([str, type, bool = true]) {
      ...
      if (str == "0") {
        ...
        if (type == "1") {
          ...
          this.cardbuild.persons = [...this.queryData];
          this.cardbuild.type = type;
        } else {
          this.cardbuild.persons = [];
          this.cardbuild.pgids = this.paramObj; //[this.cardParam.pgids];
          this.cardbuild.type = type;
        }
        if (
          (this.cardbuild.persons.length == 0 &&
            this.cardbuild.pgids.length == 0) ||
          (this.cardbuild.persons.length > 0 && this.queryDataDel.length == 0)
        ) {
          this.$message({
            message: "请先选择人员或组织",
            type: "warning",
    ...
    onClick(key, data) {
      if (key == "0") {
        ...
        if (this.$refs["paging-table"].queryData.length == 0) {
          ...
          this.$ajax(
            "/access/v2.0/auth/f/saveAccessAuthFinger",
            this.defaultData,
            "9",
            this.generateObj,
            "请求中...",
