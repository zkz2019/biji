<template>
  <fel-container title="人脸数据" @onReset="onReset" @onQuery="onQuery">
    <template slot="retrieve">
      ...
      ></fel-retrie>
      <fel-retrie v-model="param.search" :placeholder="'输入'+getNumber()+'/姓名/人脸编号查询'"></fel-retrie>
    </template>
    <paging-table
      ref="paging-table"
    ...
    <fel-popup width="80%" title="已授权门锁列表" @close="grants.box=false" :value="grants.box">
      <paging-table
        ref="grants-paging-table"
        :list="grants.list"
        interface="/face/roomfacemanage/1/listRoomFaces"
        :isAll="grants.range == 2? true:false"
        :class="{'cover-up':grants.range == 2}"
        :param="grants.param"
        ajaxType="9"
        @onSelection="(data)=>{grants.listArrs = data}"
        :refresh="grants.refresh"
        :refreshTable="grants.refreshTable"
      >
        <span class="sli">
          <el-input
            clearable
            class="search con-search"
            v-model="grants.param.search"
            placeholder="输入房间位置/名称查询"
          ></el-input>
          <fel-button type="primary" @click="grants.refresh =  new Date().getTime();">查询</fel-button>
        </span>
        <template v-if="grants.batchButs && grants.batchButs.length > 0">
          <div class="full-list" v-show="!grants.list[0].show">
            <el-checkbox
              v-model="grants.range"
              @change="onGrantsChange"
              true-label="2"
              false-label="1"
            >跨页全选</el-checkbox>
          </div>
          <batch-but
            class="sli but-blue"
            :type="grants.range"
            :list="grants.listArrs"
            :param="grants.batchButs"
            @onClick="onGrantsBatchClick"
          ></batch-but>
        </template>
      </paging-table>
      <fel-popup
        ...
        title="人脸授权"
        @close="grantsClose"
        :value="grants.grant.box"
      >
        <fel-form
          noReset
          ref="grantsForm"
          @submitForm="grantsSubmit"
          @closeForm="grantsClose"
          width="140px"
          dynamic
          :defaultData="grants.grant.data"
          :formData="grants.grant.list"
        ></fel-form>
      </fel-popup>
    </fel-popup>
    <fel-popup width="80%" title="已授权门禁列表" @close="grant.box=false" :value="grant.box">
      <paging-table
        ...
        :refreshTable="grant.refreshTable"
      >
    ...
    <fel-popup title="人脸操作记录" @close="operation.box=false" :value="operation.box">
      <paging-table
        :list="operation.list"
        interface="/face/facemanage/1/listAccessFaceManage"
      ...
      >
        <span class="sli">
          <el-input
            clearable
            class="search con-search"
            v-model="operation.param.search"
            :placeholder="'输入'+getNumber()+'/人脸编号查询'"
          ></el-input>
          <fel-button type="primary" @click="operation.refresh =  new Date().getTime();">查询</fel-button>
        </span>
      </paging-table>
      <fel-popup
        appendToBody
        title="人脸操作记录详情"
        @close="operation.details.box = false"
        :value="operation.details.box"
      >
        <!-- noInit -->
        <paging-table
          :list="operation.details.list"
          interface="/face/facemanage/2/listAccessFaceManageDetails"
          :param="operation.details.param"
          ajaxType="9"
          :refresh="operation.details.refresh"
        >
          <span class="sli">
            <el-input
              clearable
              class="search con-search"
              v-model="operation.details.param.search"
              :placeholder="'输入'+getNumber()+'/人脸编号查询'"
            ></el-input>
            <fel-button
              type="primary"
              @click="operation.details.refresh =  new Date().getTime();"
            >查询</fel-button>
          </span>
        </paging-table>
      </fel-popup>
    ...
    <fel-popup title="人员列表" @close="person.box=false" :value="person.box">
      ...
      >
        <span class="sli">
          ...
          <fel-button @click="onPersonReset">重置</fel-button>
        </span>
      </paging-table>
    </fel-popup>
...
export default {
    ...
    return {
      ...
      operation: {
          ...
          list: [
            ...
            },
            {
              ...
              value(obj) {
                ...
                return map[obj.row.afmdstatus];
              },
              formatter(obj) {
                if (obj.row.afmdstatus > 0) {
                  return 1;
                } else if (obj.row.afmdstatus < 0) {
                  return 2;
                }
                return "";
              },
        ...
        list: [
          ...
          },
          {
            ...
            click(obj) {
              $this.operation.details.param = {
                afmid: obj.row.afmid,
                search: "",
              };
              $this.operation.details.refresh = new Date().getTime();
              $this.operation.details.box = true;
            },
          },
        ],
      },
        ...
        list: [
          {
            type: "selection",
            selectable(row) {
              return $this.grants.isSelectable;
            },
          },
          {
            name: "序号",
          ...
          },
          {
            ...
            prop: "rfusecount",
            formatter(a, b, c) {
              if (a.rfusecount && a.rfusecount != 0) {
                return a.rfusecount;
              } else {
                return "不限";
              }
          ...
          },
          {
            ...
            click(obj, id, value) {
              if (id == 879) {
                let arr = (obj.row.opentime || "").split("~");
                $this.grants.grant.data = {
                  empsdate: obj.row.empsdate,
                  empedate: obj.row.empedate,
                  rfids: [obj.row.rfid],
                  openstime: arr[0] || "00:00",
                  openetime: arr[1] || "23:59",
                  opencount: obj.row.rfusecount == 0 ? "" : obj.row.rfusecount,
                  personcode: $this.grants.param.personcode,
                  search: "",
                  actiontype: 1,
                };
                $this.grants.grant.box = true;
              } else {
                $this.onGrantsDelete([obj.row.rfid]);
              }
            },
          },
        ],
      ...
      list: [
        ...
        },
        {
          ...
          click(obj, value) {
            $this.grants.param.personcode = obj.row.personcode;
            $this.grants.refresh = new Date().getTime();
            $this.grants.box = true;
          },
        },
        {
          name: "已授权门禁",
  ...
  methods: {
    ...
    toRoutes() {
      setTimeout(() => {
        if (this.toRoute && this.toRoute.personcode) {
          this.param.search = this.toRoute.personcode;
          this.onQuery();
        }
      }, 100);
    },
        ...
        .then((res) => {
          res.result.forEach((value) => {
            ...
            if (id == "871") {
              ...
              this.grants.listButs.push(obj);
              this.grants.batchButs.push(obj);
            } else if (id == "880") {
              let obj = value.entity;
              obj.alias = "删除";
              this.grant.listButs.push(obj);
    ...
    grantSubmit(data) {
      this.$ajax(
        ...
        .then((res) => {
          ...
          this.grant.range = "1";
          this.grant.refreshTable = new Date().getTime();
        })
        .catch((err) => {
          this.$message({
            showClose: true,
    ...
    grantsSubmit(data) {
      this.$ajax("/face/roomfacemanage/2/updateRoomFaces", data, "9", {}, true)
        .then((res) => {
          this.grantsClose();
          this.$message({
            message: "修改授权已下发",
          ...
          this.grants.range = "1";
          this.grants.refreshTable = new Date().getTime();
        })
        .catch((err) => {
          this.$message({
            showClose: true,
    ...
    grantsClose() {
      if (this.$refs["grantsForm"]) {
        this.$refs["grantsForm"].resetForm();
      }
      this.grants.grant.box = false;
    },
    onChange(val) {
      if (val == 2) {
    ...
    onGrantsDelete(aufids) {
      this.$confirmCon("确定要删除当前选择的授权吗？", () => {
        this.$ajax(
          "/face/roomfacemanage/3/deleteRoomFaces",
          {
            rfids: aufids,
            personcode: this.grants.param.personcode,
            search: this.grants.param.search,
            actiontype: this.grants.range,
          },
          "9",
          {},
          true
    ...
    onClick(key, obj) {
      if (key == 871) {
        ...
        this.operation.refresh = new Date().getTime();
        this.operation.param.search = "";
        this.operation.box = true;
      }
    },
    onEjectChange() {
    ...
    onGrantsChange(val) {
      if (val == 2) {
        this.$refs["grants-paging-table"].clearSelection();
        this.$refs["grants-paging-table"].toggleAllSelection();
        setTimeout(() => {
          this.grants.isSelectable = false;
        }, 100);
      } else {
        this.$refs["grants-paging-table"].clearSelection();
        this.grants.isSelectable = true;
      }
    },
    onGrantsBatchClick(key) {
      let aufids = this.grants.listArrs.map((o) => o.rfid);
      if (key == "879") {
        this.grants.grant.data = {
          empsdate: "",
          empedate: "",
