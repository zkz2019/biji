  ...
  <el-container class="wgrz312">
    ...
    <el-main class="elmin query_main">
      <paging-table
        ...
        interface="/system/control/gatelog/1/getgatelog"
        class="heig100"
        :param="param"
        :refresh="refresh"
    ...
    >
      <el-container class="dialog-table6">
        <paging-table
          interface="/system/control/gatelog/save/1/getgateway"
          class="card-paging"
        ...
        >
          <span class="sli">
            <el-input
              ...
              placeholder="输入网关通讯ID/唯一ID/创建人查询"
    ...
    >
      <el-container class="dialog-table6 query_main">
        <paging-table
          noInti
          interface="/system/control/gatelog/info/2/getgateloginfo"
          :param="journalParam"
          :refresh="journalRefresh"
          :list="journalList"
        >
          <span class="sli">
            <span class="maR5">网关</span>
            <el-select @change="onChangeJournal" class="wid150" v-model="journalParam.gateway">
              <el-option v-for="item in roomids" :key="item" :label="item" :value="item"></el-option>
            </el-select>
          </span>
      ...
      list: [
        ...
        },
        {
          ...
          template: {
            ...
            methods: {
              ...
              onClick() {
                if (this.scope.row.olcount) {
                  $this.grantParam.olid = this.scope.row.olid;
        ...
        },
        {
          ...
          template: {
            ...
            methods: {
              getClass() {
                let value = this.scope.row.olstate;
                if (value == "1") {
                  return "puc-pg";
                } else if (value == "0") {
                  return "puc-px";
            ...
            template: `<span :class='getClass()'>{{scope.row.olstatename}}</span>`,
          },
        },
        {
          name: "监控创建人",
        ...
        },
        {
          ...
          template: {
            ...
            <template v-for="(v,i) of listBut">
             <el-button v-show="isShow(row.olstate,v.type)" :key="i" type="text" size="small" @click.stop="onClick(v.type, v)">{{v.name}}</el-button>
            </template>
            </div>`,
          },
        },
  ...
  methods: {
    ...
    submitForm() {
      if (this.gateway && this.gateway.length > 0) {
        this.$ajax(
          "/system/control/gatelog/save/2/savegatelog",
          {
            olremark: this.olremark,
          },
          "1",
          this.gateway.map((o) => o.gatewaycode),
          true
        )
          .then((res) => {
            this.closeForm();
    ...
    onClick(key, data, obj) {
      if (key == "6") {
        this.$confirm("此操作将此启用此日志，是否继续？", "提示", {
          ...
          .then(() => {
            this.$ajax(
              "/system/control/gatelog/regain/1/regaingatelog",
              {
                olid: data.olid,
              },
              "1",
        ...
        this.$confirm("此操作将此停用此日志，是否继续？", "提示", {
          ...
          .then(() => {
            this.deleteJournal([data.olid], obj.alias);
          })
          .catch((err) => {});
      } else if (key == "3") {
        this.$ajax(
          "/system/control/gatelog/info/1/getgateinfo",
          {
            olid: data.olid,
          },
          "1"
          ...
          .then((res) => {
            ...
            this.journalParam.olid = data.olid;
            this.journalParam.gateway = this.roomids[0] || "";
            this.journalRefresh = new Date().getTime();
    ...
    deleteJournal(data, ts) {
      this.$ajax(
        "/system/control/gatelog/delete/1/deletegatelog",
        { olid: data },
        "1",
        {},
        true
    ...
    onEjectChange() {
      this.$common.onEjectChange(this.list, "wgyxjk312");
    },
    getEject() {
      this.$common.getEject(this, "list", "wgyxjk312");
    },
  },
};
</script>