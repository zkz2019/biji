<template>
  <div class="msInfo">
    ...
    <paging-table
      ...
      interface="/login/main/4/listPersonAccess"
      :list="list"
      :refresh="refresh"
...
export default {
  ...
  created() {
    ...
    console.log("this.buts", this.buts);
    this.buts.forEach((item) => {
      if (item.entity.id == "971") {
        this.batchButs.push(item.entity);
        //重载
      } else if (item.entity.id == "972") {
        this.batchButs.push(item.entity);
        //修改
      } else if (item.entity.id == "973") {
        this.batchButs.push(item.entity);
        //删除
      } else if (item.entity.id == "974") {
        //操作记录
  ...
  methods: {
    getData() {
      ...
      this.$ajax(
        ...
        console.log("res", res);
        this.authaccessfail = res.result.authaccessfail;
      });
    },
    getType() {
