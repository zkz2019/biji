...
<template>
  ...
  >
    <el-container class="dialog-table6 query_main">
      <paging-table
        interface="/person/7/getpersonauthdel"
        ref="paging-table"
        :list="list"
        :param="param"
        :refresh="refresh"
      >
        <span class="sli">
          <el-input clearable class="search" v-model="param.search" :placeholder="'输入账号/姓名查询'"></el-input>
...
import delListDetails from "./delListDetails";
export default {
  props: {
    ...
    return {
      ...
      list: [
        ...
        },
        {
          ...
          formatter(row, b, c, d) {
            if (row) {
              let newtotal = Number(row.padnocount) + Number(row.padokcount);
              let total = Number(row.padallcount);
