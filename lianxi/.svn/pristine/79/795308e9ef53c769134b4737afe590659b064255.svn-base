<template>
  <el-main class="roomBox" v-loading="loading">
        ...
        <el-collapse-item v-for="(obj, key) of list" :key="key" :name="key + ''">
          <template slot="title">
            <div class="roomBox_title">
              <div>
                <!-- <span
                  v-if="getTextObj(obj).Show v-if="ssList.length>0""
                ...
                <el-popover placement="left-start" width="350" trigger="hover">
                  <div class="layerList_details">
                    <div v-if="getssList(obj).length > 0">
                      <h4>{{ getRoomtype2s[1] || '宿舍房间' }}</h4>
                      <p v-for="item in getssList(obj)">{{ item.name + ' : ' + item.num }}</p>
                    </div>
                    <div v-if="getggList(obj).show">
                      <h4>{{ getRoomtype2s[2] || '公共房间' }}</h4>
                      <p v-for="item in getggList(obj).ggList">{{ item.name + ' : ' + item.num }}</p>
                    </div>
                    <div v-if="getjdList(obj).length > 0">
                      <h4>{{ getRoomtype2s[4] || '酒店房间' }}</h4>
                      <p v-for="item in getjdList(obj)">{{ item.name + ' : ' + item.num }}</p>
                    </div>
                  </div>
                  <span slot="reference">
                    <!-- <i class="ficon-image28" style="color:#24a6f9" @click.stop="onDetails(obj)"></i> -->
                    {{ getTextObj(obj).text }}
                  </span>
                </el-popover>
              </div>
                ...
                <img
                  :src="
                    activeNames.includes(key + '')
                      ? require('../../assets/image/open-right.png')
                      : require('../../assets/image/open-bottom.png')
                  "
                  alt
                />
              </div>
                    ...
                    >
                      ...
                      <div>
                        <img
                          ...
                          :title="setTitle(item)"
                        />
                        <div :title="setTitle(item)" class="roomtext">
                          {{
                          item.roomtype2 != 2
                          ? /[\u4E00-\u9FA5]|[\uFE30-\uFFA0]/gi.test(
                          item.roomname,
                          )
                          ? item.roomname.substring(0, 3)
                          : item.roomname
                          : ''
                          }}
                        </div>
                      ...
                      <div
                        :class="{
                          Danger: item.roomtype == 4 && item.roomrzperson != 0,
                        }"
                        v-if="
                          item.roomtype2 == 1 &&
                          item.roomnetquality != '0' &&
                          item.roomnetquality != '-2' &&
                          (item.roomtype == '1' || item.roomtype == '4')
                        "
                      >
                        {{
                        item.roomtype == 1 || item.roomtype == 4
    ...
    >
      <div class="layerList_details">
        <div v-if="ssList.length>0">
          <h4>宿舍房间</h4>
          <p v-for="item in ssList">{{item.name+" : "+item.num}}</p>
        </div>
        <div v-if="ggList.length>0">
          <h4>公共房间</h4>
          <p v-for="item in ggList">{{item.name+" : "+item.num}}</p>
        </div>
        <div v-if="jdList.length>0">
          <h4>酒店房间</h4>
          <p v-for="item in jdList">{{item.name+" : "+item.num}}</p>
        </div>
      </div>
    </el-dialog>-->
  </el-main>
...
export default {
  ...
  created() {
    ...
    console.log("this.roomnexttypes", this.roomnexttypes[1]);
  },
  watch: {
    value3(val) {
  ...
  methods: {
    ...
    getssList(obj) {
      return [
        { name: "床位数", num: obj.roommaxcount },
        { name: "入住人数", num: obj.roomrzcount }
      ];
    },
    getggList(obj) {
      let ggList = [
        { name: this.roomnexttypes[1] || "教室", num: 0 },
        { name: this.roomnexttypes[2] || "办公室", num: 0 },
        { name: this.roomnexttypes[3] || "会议室", num: 0 },
        { name: this.roomnexttypes[4] || "实验室", num: 0 }
      ];
      let show = false;
      obj.roomllist.forEach(item => {
        if (item.roomtype == 2) {
          show = true;
          switch (item.roomnexttype) {
            case 1:
              ggList[0].num++;
              break;
            case 2:
    ...
    onDetails(obj) {
      ...
      console.log("obj", this.ggList, this.ssList, obj);
    },
    ...
    getTextObj(obj) {
      ...
      if (Proom > 0) {
        text = (this.getRoomtype2s[2] || "公共房间") + `:${Proom}`;
      }
      if (Sroom > 0) {
        if (Proom > 0) {
          text += ` | `;
        }
        text += (this.getRoomtype2s[1] || "宿舍房间") + `:${Sroom}`;
      }
      if (Jroom > 0) {
        if (Proom > 0 || Sroom > 0) {
          text += ` | `;
        }
        text += (this.getRoomtype2s[4] || "酒店房间") + `:${Jroom}`;
      }
      // if (Sroom > 0 || Jroom > 0) {
      //   text += ` | `;
