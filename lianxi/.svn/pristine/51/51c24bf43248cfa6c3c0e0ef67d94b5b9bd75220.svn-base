...
export default {
  ...
  methods: {
    ...
    onChange(val, data) {
      ...
      this.formData.slice(1, 12).forEach((item, ind) => {
        ...
        if (val == "3" && ind == 0) {
          ...
          this.defaultData = {
            lotordertype: "22",
            iscleari: this.getIsFace() ? "1" : "0",
            isclearcp: this.getIsCard() || this.getIsPwd() ? "1" : "0",
            isclearf: this.getIsFinger() ? "1" : "0",
            isclearb: this.getIsApp() ? "1" : "0",
          };
          if (ind == 7) {
            item.noShow = this.getIsCard() || this.getIsPwd() ? false : true;
          } else if (ind == 8) {
            item.noShow = this.getIsFinger() ? false : true;
          } else if (ind == 9) {
            item.noShow = this.getIsApp() ? false : true;
          } else if (ind == 10) {
            item.noShow = this.getIsFace() ? false : true;
          }
        } else {
          item.noShow = true;
        }
    ...
    async submitForm(data) {
      let a = data.isclearb == "1" ? "蓝牙" : "";
      let b = data.isclearcp == "1" ? "卡密" : "";
      let c = data.isclearf == "1" ? "指纹" : "";
      let d = data.iscleari == "1" ? "人脸" : "";
      this.text =
        a +
        (a && b ? "/" + b : b) +
        (b && c ? "/" + c : c) +
      ...
      if (this.build.length > 0 || this.areas.length > 0) {
        if (data.lotordertype == "1") {
          ...
          msg = "批量远程关门";
        } else if (data.lotordertype == "21") {
          url = "/lock/lotorder/save/h/savelotfacesync"; //h 批量人脸同步
          msg = "批量人脸同步";
          ...
          msg = "批量开放时间控制";
        } else if (data.lotordertype == "22") {
          url = "/lock/lotorder/save/i/savelotclear"; //批量清空授权
          msg = "批量清空授权";
          if (
            data.isclearb == "0" &&
            data.isclearcp == "0" &&
            ...
            this.types = {
              isclearb: data.isclearb,
              isclearcp: data.isclearcp,
              isclearf: data.isclearf,
              iscleari: data.iscleari,
            };
            if (
              data.isclearb == "1" &&
              data.isclearcp == "0" &&
              ...
              this.types.isclearserver = "1";
            } else {
              let k = await this.checkLocal();
              if (k.next) {
                this.types.isclearserver = k.isclearserver;
              } else {
                return;
              }
            }
