      ...
      <el-container :key="key1" v-loading="loading1" class="checkIn_right">
        <el-header class="checkIn_right_top query_head">
          <template v-if="btnRightShow.show725">
            ...
            <!-- <el-input  placeholder="输入学号|卡号|姓名|身份证" clearable class="con-search qh_inp" v-model="value"></el-input> -->
...
export default {
        ...
        },
        {
          ...
          render(obj, add) {
            return add(
              ...
              {
                props: {
                  selectShow: !obj.passdisabled,
                  tjShow: $this.passaddHide1,
                  scShow: $this.passaddHide,
                  allShow:
                    $this.passaddHide1 || $this.passaddHide ? true : false,
                  addShow: $this.btnRightShow.show807,
                  delShow: $this.btnRightShow.show808,
                  input:
                    obj.isaddpass == "0"
                      ? $this.isLeft
                        ? "未下发"
                      ...
                      ? "已授权"
                      : "",
                  defaultVal: $this.isselect.isselectpass
                    ? String($this.isselect.isselectpass)
                    : "",
                },
                on: {
                  onClick(val) {
                    if (val == "1") {
                      $this.rightAdd("5", "修改密码权限");
                    } else {
                      $this.rightDelete("5", "修改密码权限");
                    }
                  },
                  onChangeSel(val) {
                    obj.passaddVal = val;
        ...
        },
        {
          ...
          render(obj, add) {
            return add(
              ...
              {
                props: {
                  selectShow: !obj.sweepdisabled,
                  tjShow: $this.sweepHide1,
                  scShow: $this.sweepHide,
                  allShow: $this.sweepHide1 || $this.sweepHide ? true : false,
                  input:
                    obj.issweep == "0"
                      ? $this.isLeft
                        ? "未下发"
                      ...
                      ? "已授权"
                      : "",
                  defaultVal: $this.isselect.isselectsweep
                    ? String($this.isselect.isselectsweep)
                    : "",
                },
                on: {
                  onClick(val) {
                    if (val == "1") {
                      $this.rightAdd("6", "开门二维码");
                    } else {
                      $this.rightDelete("6", "开门二维码");
                    }
                  },
                  onChangeSel(val) {
                    obj.sweepVal = val;
...

        {
          ...
          render(obj, add) {
            return add(
              ...
              {
                props: {
                  selectShow: Boolean(!obj.iddisabled),
                  tjShow: Boolean($this.idcardHide1 && obj.ishaveidcard),
                  addShow: $this.btnRightShow.show727,
                  delShow: $this.btnRightShow.show728,
                  scShow: $this.idcardHide,
                  allShow:
                    Boolean($this.idcardHide1 && obj.ishaveidcard) ||
                    $this.idcardHide
                      ? true
                      : false,
                  issendText: obj.idcardissend,
                  input:
                    obj.idcardcode == "未授权"
                      ? obj.ishaveidcard
                        ? "未下发"
                        : "身份证未录入"
                      : obj.idcardcode,
                  defaultVal: $this.isselect.isselectidcard
                    ? String($this.isselect.isselectidcard)
                    : "",
                },
                on: {
                  onClick(val) {
        ...
        },
        {
          ...
          render(obj, add) {
            return add(
              ...
              {
                props: {
                  selectShow: Boolean(!obj.facedisabled),
                  tjShow: Boolean($this.faceHide1 && obj.ishaveface),
                  addShow: true,
                  delShow: true,
                  scShow: $this.faceHide,
                  allShow:
                    Boolean($this.faceHide1 && obj.ishaveface) || $this.faceHide
                      ? true
                      : false,
                  issendText: obj.faceissend,
                  input:
                    obj.facecount == "未授权"
                      ? obj.ishaveface == 1
                        ? "未下发"
                      ...
                      ? "已授权"
                      : "",
                  defaultVal: $this.isselect.isselectface
                    ? String($this.isselect.isselectface)
                    : "",
                },
                on: {
                  onClick(val) {
  ...
  mounted() {
    ...
    console.log("this.checkedroom", this.checkedroom, this.getIsPwd());
    if (this.getIsCard()) {
      if (this.checkedroom.iscardlock == 0) {
        this.tableData[8].noShow = true;
        this.tableData[13].noShow = true;
      } else {
        this.tableData[8].noShow = false;
        this.tableData[13].noShow = false;
      }
    } else {
      this.tableData[8].noShow = true;
      this.tableData[13].noShow = true;
    }
    if (this.checkedroom.isidcardlock == 0) {
      this.tableData[15].noShow = true;
    } else {
      this.tableData[15].noShow = false;
    }
    if (this.getIsPwd()) {
    ...
    if (this.getIsFace()) {
      if (this.checkedroom.isfacelock == 0) {
        this.tableData[16].noShow = true;
      } else {
        this.tableData[16].noShow = false;
      }
    } else {
      this.tableData[16].noShow = true;
    }
    if (this.getIsFinger()) {
      if (this.checkedroom.isfingerentry == 0) {
        this.tableData[14].noShow = true;
      } else {
        this.tableData[14].noShow = false;
      }
    } else {
      this.tableData[14].noShow = true;
    }
  ...
  methods: {
    getmunu() {
      this.btnRight.forEach((item) => {
        ...
        if (id == "725") {
          ...
          this.btnRightShow.show807 = true;
        } else if (id == "808") {
          this.btnRightShow.show808 = true;
        }
      });
    },
    onInstall(is) {
    ...
    Udates(obj) {
      ...
      this.$confirm("确定要修改入住开始结束时间吗？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(() => {
      ...
      this.$ajax(
        ...
        .then((res) => {
          ...
          let obj = {
            ...
            isaddpass: res.result.isaddpass,
            idcardcode: res.result.idcardcode,
            facecount: res.result.facecount,
            ishaveface: res.result.ishaveface,
          ...
          this.sweepHide = res.result.issweep == "1" ? true : false;
          this.apptryHide = res.result.isfingerentry == "1" ? true : false;
          this.passaddHide = res.result.isaddpass == "1" ? true : false;
          this.cardHide = res.result.cardcode == "未授权" ? false : true;
          this.idcardHide = res.result.idcardcode == "未授权" ? false : true;
          this.faceHide = res.result.facecount == "未授权" ? false : true;
          this.fingerHide1 = res.result.fingercount == "未授权" ? true : false;
          this.appHide1 = res.result.appcount == "未授权" ? true : false;
          this.sweepHide1 = res.result.issweep == "1" ? false : true;
          this.apptryHide1 = res.result.isfingerentry == "1" ? false : true;
          this.passaddHide1 = res.result.isaddpass == "1" ? false : true;
          this.cardHide1 = res.result.cardcode == "未授权" ? true : false;
          this.idcardHide1 = res.result.idcardcode == "未授权" ? true : false;
          this.faceHide1 = res.result.facecount == "未授权" ? true : false;
          this.loading1 = false;
          this.key1++;
          this.value = "";
          this.list = [];
    ...
    Determine() {
      ...
      if (
        ...
        this.defaultData.sweepVal == "1" ||
        this.defaultData.idcardVal == "1" ||
        this.defaultData.faceVal == "1"
      ) {
        let obj = {
          ...
          issendsweep: Number(this.defaultData.sweepVal),
          issendfingerentry: this.defaultData.apptryVal,
          issendaddpass: this.defaultData.passaddVal,
          issendidcard: this.defaultData.idcardVal,
          issendface: this.defaultData.faceVal,
          idcardcode: this.defaultData.idcardcode,
          opentime: "",
          password: this.input,
    ...
    onEmpty() {
      ...
      this.tzHide = false;
      this.cardHide = false;
      this.idcardHide = false;
      this.faceHide = false;
      this.passHide = false;
      this.fingerHide = false;
      this.appHide = false;
      this.sweepHide = false;
      this.apptryHide = false;
      this.passaddHide = false;
      this.appHide1 = false;
      this.sweepHide1 = false;
      this.apptryHide1 = false;
      this.passaddHide1 = false;
      this.fingerHide1 = false;
      this.cardHide1 = false;
      this.idcardHide1 = false;
      this.faceHide1 = false;
    ...
    onChange(/*val*/) {
      ...
      this.defaultData.idcardissend = "";
      this.defaultData.faceissend = "";
      this.defaultData.passissend = "";
      this.defaultData.fingerissend = "";
      this.$set(this.defaultData, "cardVal", this.isselect.isselectcard);
      this.$set(this.defaultData, "idcardVal", this.isselect.isselectidcard);
      this.$set(this.defaultData, "faceVal", this.isselect.isselectface);
      this.$set(this.defaultData, "appVal", this.isselect.isselectapp);
      this.$set(this.defaultData, "sweepVal", this.isselect.isselectsweep);
      this.$set(this.defaultData, "passaddVal", this.isselect.isselectpass);
      this.$set(
        this.defaultData,
        "apptryVal",
      ...
      if (this.list[0].isclosecard == 1) {
        //判断是否隐藏卡片选项
        this.tableData[8].noShow = true;
      } else {
        this.tableData[8].noShow = false;
      }
      if (this.checkedroom.isclosepass == 1) {
        //判断是否隐藏密码选项
        this.tableData[9].noShow = true;
        this.tableData[10].noShow = true;
      } else {
      ...
      if (this.list[0].iscloseidcard == 1) {
        //判断是否隐藏身份证选项
        this.tableData[15].noShow = true;
      } else {
        this.tableData[15].noShow = false;
      ...
      (this.passaddHide1 = false), (this.passDeleteHide = false);
      this.apptryHide = false;
      this.passaddHide = false;
      this.getbedno();
      let obj = {
        ...
        passdisabled: false,
        dates: [String(format(this.timeStary, "yyyy-MM-dd HH:mm:ss")), ""],
        bednoVal: "",
      };
      ...
      if (this.list[0] != undefined) {
        ...
        if (this.list[0].isidcard == 0) {
          obj.iddisabled = true;
          this.defaultData.idcardVal = "";
        } else {
          obj.iddisabled = false;
        }
        if (this.list[0].isface == 0) {
          obj.facedisabled = true;
          this.defaultData.faceVal = "";
        } else {
          obj.facedisabled = false;
        }
        ...
        if (this.list[0].issweep == "") {
          obj.sweepdisabled = true;
          this.defaultData.sweepVal = "";
        } else {
          obj.sweepdisabled = false;
        }

        if (this.list[0].isfingerentry == "") {
          obj.apptrydisabled = true;
          this.defaultData.apptryVal = "";
        } else {
          obj.apptrydisabled = false;
        }
        if (this.list[0].isaddpass == "") {
          obj.passdisabled = true;
          this.defaultData.passaddVal = "";
        } else {
          obj.passdisabled = false;
        }
      }
