<!-- 楼层效果弹出框 -->
<template>
  <div class="effect-floor classify-classify">
    ...
    <div v-if="roomlist.length > 1" class="room_buts">
  ...
  methods: {
    ...
    onReverse(key) {
      if (key == 1 && this.array.length > 1) {
        ...
        this.roomlist = arrSort(this.array, "areaname");
        this.array = [...this.roomlist];
        this.key = new Date().getTime();
        setTimeout(() => {
          this.setSortable();
