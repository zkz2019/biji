  ...
  <el-container>
    <el-header class="elheader query_headbox">
      ...
      <retrieval class="query_head">
        <inpbox :inpb="true">
          <el-select v-model="param.cardstate" class="con-select qh_inp">
            <el-option
        ...
        <inpbox :inpb="true">
          <el-select v-model="param.cardtype" class="con-select qh_inp">
            <el-option
              v-for="item in types"
              :key="item.cardtype"
        ...
        <inpbox :inpb="true">
          <el-input
            ...
            clearable
            type="text"
            :placeholder="'输入卡号/'+getNumber()+'/姓名/手机号查询'"
    ...
    <!-- <guideFile
      ...
      detailsUrl="/auth/cardcenter/card/uploadcard/7/downcardhistorydetails"
      historyUrl="/auth/cardcenter/card/uploadcard/6/downcardhistory"
      failUrl="/auth/cardcenter/card/uploadcard/8/downcardexcel"
    ...
    <outList class="personnel_list" @beforeClose="outVisible=false" :dialogVisible="outVisible"></outList>
  </el-container>
</template>
...
// import guideFile from "../personnel/guideFile.vue";
export default {
  name: "kpgl384",
  components: {
    guideFile,
      ...
      bottomBtn: [
        {
          ...
          listInfo: [
            ...
            },
            {
              ...
              template: {
                ...
                computed: {
                  name() {
                    if (this.scope.row.ucstatus == "0") {
                      return "正在导入";
                    } else if (this.scope.row.ucstatus == "-1") {
                      return "失败";
                    } else {
                      return "成功";
                    }
                ...
                methods: {
                  getClass() {
                    let value = this.scope.row.ucstatus;
                    if (value == "1") {
                      return "puc-pg";
                    } else if (value == "-1") {
                      return "puc-px";
          ...
          url: "/auth/cardcenter/card/uploadcard/a/getUploadCard",
          interface: "/auth/cardcenter/card/uploadcard/b/getUploadCardDetails",
        },
      ],
      mjList: [
        {
        ...
        },
        {
          name: "开门时间段",
          formatter(row) {
            return row.aucopenstime + " - " + row.aucopenetime;
          },
        },
        {
          name: "可开门次数",
      ...
      list: [
        ...
        },
        {
          ...
          template: {
            ...
            template: `<div>
              <span v-if="isDisabled(scope.row.accesscount)"><a class="a-click" @click.stop="onClick">{{scope.row.accesscount}}</a></span>
  ...
  computed: {
    interface() {
      return this.ismj
        ? "/access/v2.0/auth/info/3/listCardAccessAuth"
        : "/auth/cardcenter/card/deletecardauths/4/getcardauth";
    },
  },
  mounted() {
    this.getEject();
  ...
  methods: {
                ...
                arr.forEach((obj) => {
                  ...
                  if (vid == "392") {
                    this.importButs.push({
                      name: valias,
                      url: "/auth/cardcenter/card/uploadcard/d/uploadCards",
                      errUrl:
                        "/auth/cardcenter/card/uploadcard/c/downUploadCardFail",
                      progUrl:
                        "/auth/cardcenter/card/uploadcard/9/getUploadRate",
                    });
                    // this.importButs.push({
                    //   name: valias,
                    //   tempUrl:
                    ...
                    this.exportButs.push({
                      id: vid,
                      name: valias,
                      url: "/auth/cardcenter/card/uploadcard/5/downcards",
                      data: this.param,
                    });
                  } else if (vid == "541") {
                    this.exportButs.push({
    ...
    resultdata(data) {
      //读卡
      if (data.err === 0 || data.type === "Result") {
        if (this.dialogAdd) {
          this.$refs.cardAdd.resultdata(data);
        } else {
    ...
    onGrantChange(val) {
      if (val == 2) {
        if (this.$refs["grant-paging-table"]) {
    ...
    confirmModify(data) {
      if (this.ismj) {
        let adids = this.modifyObj.map((o) => o.adid);
        let obj = {
          adids,
          aucempedate: data.aucempedate,
          aucempsdate: data.aucempsdate,
          aucopenetime: data.aucopenetime,
          aucopenstime: data.aucopenstime,
          aucusecount: data.aucusecount,
          cardcode: this.grantParam.cardcode,
          type: this.grantRange,
        };
        this.$ajax("/access/v2.0/auth/info/5/updateCardAccessAuth", obj, "9")
          .then((res) => {
            this.onGrantRefreshTable();
      ...
      if (this.grantParam && this.grantParam.cardcode) {
        if (this.grantRange != 2) {
          if (data && data.length > 0) {
            if (data.length == 1) {
              if (this.ismj) {
                let aucusecount = data[0].count;
                if (!/^[0-9]*$/.test(aucusecount)) {
                  aucusecount = "";
                }
                let aucopenstime = data[0].aucopenstime;
                if (!/^[:\s0-9\-]*$/.test(aucopenstime)) {
                  aucopenstime = "";
                }
                let aucopenetime = data[0].aucopenetime;
                if (!/^[:\s0-9\-]*$/.test(aucopenetime)) {
                  aucopenetime = "";
                }
                let aucempsdate = data[0].aucempsdate;
                if (!/^[:\s0-9\-]*$/.test(aucempsdate)) {
                  aucempsdate = "";
                }
                let aucempedate = data[0].aucempedate;
                if (!/^[:\s0-9\-]*$/.test(aucempedate)) {
                  aucempedate = "";
                }
                this.defaultData = {
                  aucempsdate: aucempsdate,
                  aucempedate: aucempedate,
                  aucopenstime: aucopenstime,
                  aucopenetime: aucopenetime,
                  aucusecount: aucusecount,
                };
              } else {
    ...
    onDelMj(obj) {
      ...
      this.$confirmCon("确定删除当前卡片吗？", () => {
        this.$ajax("/access/v2.0/auth/info/4/delCardAccessAuth", data, "9")
          .then((res) => {
            this.onGrantRefreshTable();
            this.$message({
              message: "删除成功!",
    ...
    onGrantBatchClick(key) {
      if (this.grantRange == 1 && this.rcids.length == 0) {
        ...
        if (key == 395) {
          this.ismj ? this.onDelMj() : this.onGrantDelete([], key);
        } else if (key == 396) {
          // this.ismj ? this.onModifyMj() :
          this.onCardModify();
        }
      }
    },
    onClick(key, data, obj) {
      if (key == 4) {
        ...
        data.opencount = data.opencount;
        const openTime =
          data && data.cardunlocktime && data.cardunlocktime.split("~");

        this.defaultDataAdd = Object.assign(
          {
            openstime: openTime[0] || "00:00",
            openetime: openTime[1] || "23:59",
          },
          data
        );
      } else if (key == "g2") {
        ...
        this.ismj ? this.onDelMj(data) : this.onGrantDelete([data], key);
      } else if (key == "896") {
