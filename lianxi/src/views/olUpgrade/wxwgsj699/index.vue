...
<template>
  <el-container>
    ...
    <wxAutoup :stopBtns="stopBtns" :dialogVisible="autoVisible" @beforeClose="autoVisible = false"></wxAutoup>
    <history
...
import wxAutoup from "./wxAutoup";
import history from "./history";
export default {
  components: { wxVup, wxAutoup, history },
  props: {
    toParam: Object,
    toRoute: Object,
  },
          ...
          template: {
            ...
            computed: {
              listBut() {
                ...
                list.forEach((item) => {
                  item.show = item.id == "717" ? false : show;
                  if (
                    item.id == "700" ||
                    item.id == "701" ||
                    item.id == "705"
                    ...
                    lists[0].push(item);
                  } else if (item.id == "702" || item.id == "703") {
                    lists[1].push(item);
                  } else if (item.id == "721" || item.id == "717") {
                    lists[2].push(item);
                  }
                });
                return lists;
  ...
  methods: {
    ...
    getType() {
      this.$ajax(
        "/system/onlineup/wxgateversion/5/getsearchtype",
        {},
        "1"
      ).then((res) => {
    ...
    onStartnblockup(type, row = null) {
      ...
      switch (type) {
        case "1":
          text = "全新下载";
          url = "/system/onlineup/savewxgateversion/2/startwxgateup"; //全新下载
          break;
        case "2":
          text = "续传下载";
          url = "/system/onlineup/savewxgateversion/4/restartwxgateup"; // 续传下载
          break;
        case "3":
          text = "更新升级";
        ...
        case "5":
          text = "停止下载";
          url = "/system/onlineup/savewxgateversion/6/deletewxgateup"; //停止下载
          break;
        case "7":
          text = "状态查询";
        ...
        case "8":
          text = "自动升级";
          url = "/system/onlineup/savewxgateversion/8/saveautogateup"; //自动升级
          break;
      }
      let data = {
      ...
      this.$ajax("/login/home/2/getsonmenu", { fatherid: id }, "1")
        .then((res) => {
          res.result.forEach((item) => {
            ...
            } else if (entity.id == "722") {
              this.topButs.push(entity);
            } else if (entity.id == "930") {
    ...
    onEjectChange() {
      this.$common.onEjectChange(this.list, "wxwgsj699");
    },
    getEject() {
      this.$common.getEject(this, "list", "wxwgsj699");
    },
  },
};
</script>
