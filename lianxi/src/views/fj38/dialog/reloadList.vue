...
<template>
  ...
  >
    <el-container class="dialog-table6 query_main">
      <paging-table
        interface="/lock/d/listBuildAuthReload"
        ref="paging-table"
        :list="list"
        ajaxType="9"
        :param="param"
...
import reloadListDetails from "./reloadListDetails";
export default {
  props: {
    ...
    return {
      ...
      list: [
        ...
        },
        {
          ...
          formatter(row, b, c, d) {
            if (row) {
              let newtotal = Number(row.barnocount) + Number(row.barokcount);
              let total = Number(row.barallcount);
