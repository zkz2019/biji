  ...
  <el-container>
    <el-header class="query_headbox">
      <retrieval class="query_head">
        ...
        <inpbox :inpb="true">
          <el-select class="qh_w270 qh_inp" placeholder="全部指令类型" v-model="param.lotordertype">
            <el-option v-for="item in options" :key="item.id" :label="item.text" :value="item.id"></el-option>
          </el-select>
        </inpbox>
        <inpbox :inpb="true">
          <el-input clearable class="qh_w270 qh_inp" v-model="param.search" :placeholder="'输入账号查询'"></el-input>
        </inpbox>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      dates: [],
      param: { sdate: "", edate: "", search: "", lotordertype: "0" },
      options: [],
      options1: [], //弹框数据
      coid: "",
  ...
  computed: {
    listBut() {
      let all = Storages.getStorage("plxfBtn");
      let but = [];
