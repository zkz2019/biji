          ...
          <el-popover placement="left-start" width="350" trigger="hover">
            <div class="layerList_details">
              <div v-if="getssList().length>0">
                ...
                <p v-for="(item,key) in getssList()" :key="key">{{item.name+" : "+item.num}}</p>
              </div>
              <div v-if="getggList().show">
                <h4>{{getRoomtype2s[2]||"公共房间"}}</h4>
                <p v-for="(item,key) in getggList().ggList" :key="key">{{item.name+" : "+item.num}}</p>
              </div>
              <div v-if="getjdList().length>0">
                <h4>{{getRoomtype2s[4]||"酒店房间"}}</h4>
                <p v-for="(item,key) in  getjdList()" :key="key">{{item.name+" : "+item.num}}</p>
              </div>
            </div>
            <p
              slot="reference"
  ...
  methods: {
    ...
    getTextObj() {
      ...
      if (obj.publiccount > 0) {
        num += 1;
        text = (this.getRoomtype2s[2] || "公共房间") + `: ${obj.publiccount}`;
      }
      // if (obj.labcount > 0) {
      //   if (num > 0) {
      //     text += ` | `;
      ...
      if (obj.roomcount > 0) {
        ...
        num += 1;
        text += (this.getRoomtype2s[1] || "宿舍房间") + `: ${obj.roomcount}`;
      }
      if (obj.jdcount > 0) {
        if (num > 0) {
          text += ` | `;
        ...
        num += 1;
        text += (this.getRoomtype2s[4] || "酒店房间") + `: ${obj.jdcount}`;
      }
